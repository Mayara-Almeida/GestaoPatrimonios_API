...
namespace GestaoPatrimonios.Applications.Services
{
    public class AutenticacaoService
    {
        ...
        {
            Usuario usuario = _repository.ObterPorNIFComTipoUsuario(loginDto.NIF); // Verifica se o usuário existe 
        ...
        public void TrocarPrimeiraSenha(Guid usuarioId, TrocarPrimeiraSenhaDto dto)
        {
            ...
            if (usuario == null)
            {
                throw new DomainException("Usuátio não encontrado.");
            }
            ...
            usuario.Senha = CriptografiaUsuario.CriptografarSenha(dto.NovaSenha); // Pega a senha criptografa e substitui pela senha que está no banco
            usuario.PrimeiroAcesso = false; //A senha do usuário não é mais seu nif pois ele já acessou antes
